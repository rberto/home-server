import time
import os
from BMP085 import Bmp085
import argparse

# Parsing command line arguments.
parser = argparse.ArgumentParser()
parser.add_argument("--interval", type=int, help="interval between measurements in seconds.", default=30)
parser.add_argument("--period", type=int, help="Time in seconds for the program to continue measurement.", default=600)
args = parser.parse_args()

# I2C address of the BMP085 sensor.
SENSOR_ADDRESS = 0x77
temps = []
pressures = []
start_time = time.time()

print "Initialize sensor on I2C address: %s" % SENSOR_ADDRESS
bmp085 = Bmp085(SENSOR_ADDRESS, 0)

while time.time() - startdate < args.period:
    # read both temp and pressure from sensor.
    (temp, p) = bmp085.readTempPressure()
    # add read values to buffer lists.
    temps.append(temp)
    pressures.append(p)
    # Wait for the set time between intervals.
    time.sleep(args.interval)

key = time.time()
now = datetime.now()
values = [key, now.year, now.month, now.day, now.hour, now.minute, now.second]
values.append(sum(temps)/len(temps))
values.append(sum(pressures)/len(pressures))
# Convert list to list of str.
values = ["%s" % x for x in values]
# Connect to database.
dbconnection = sqlite3.connect('/home/pi/db/temppressure.db')
cursor = dbconnection.cursor()
# Insert data into database.
cursor.execute("INSERT INTO data VALUES (?,?,?,?,?,?,?,?,?)", values)
dbconnection.commit()
cursor.close()
dbconnection.close()
